/**
 * UI Translations
 * Structure: { key: { en: "...", nl: "...", de: "..." } }
 */
const uiTranslations = {
    // Navigation
    "nav_home": {
        en: "Home",
        nl: "Home",
        de: "Startseite"
    },
    "nav_about": {
        en: "About",
        nl: "Over ons",
        de: "Über uns"
    },
    "nav_showcase": {
        en: "Research",
        nl: "Onderzoek",
        de: "Forschung"
    },
    "nav_contact": {
        en: "Contact",
        nl: "Contact",
        de: "Kontakt"
    },

    // Hero Section
    "hero_title": {
        en: "You describe the problem.<br><span>We build the software.</span>",
        nl: "Jij beschrijft het probleem.<br><span>Wij bouwen de software.</span>",
        de: "Sie beschreiben das Problem.<br><span>Wir bauen die Software.</span>"
    },
    "hero_input_label": {
        en: "Your Input",
        nl: "Jouw Input",
        de: "Ihr Input"
    },
    "hero_input_desc": {
        en: "Specific needs",
        nl: "Specifieke wensen",
        de: "Spezifische Anforderungen"
    },
    "hero_solution_label": {
        en: "Our Solution",
        nl: "Onze Oplossing",
        de: "Unsere Lösung"
    },
    "hero_solution_desc": {
        en: "Custom made<br>software",
        nl: "Op maat gemaakte<br>software",
        de: "Maßgeschneiderte<br>Software"
    },
    "hero_outcome_label": {
        en: "The Outcome",
        nl: "Het Resultaat",
        de: "Das Ergebnis"
    },
    "hero_outcome_desc": {
        en: "Problem solved",
        nl: "Probleem opgelost",
        de: "Problem gelöst"
    },

    // About Section
    "about_title": {
        en: "Tailored for Your Business",
        nl: "Op Maat voor Jouw Bedrijf",
        de: "Maßgeschneidert für Ihr Unternehmen"
    },
    "about_p1": {
        en: "<strong>RobaLink</strong> builds custom software for companies that need a specific tool, system, or AI integration to make their work easier or more reliable.",
        nl: "<strong>RobaLink</strong> bouwt software op maat voor bedrijven die een specifieke tool, systeem of AI-integratie nodig hebben om hun werk eenvoudiger of betrouwbaarder te maken.",
        de: "<strong>RobaLink</strong> entwickelt kundenspezifische Software für Unternehmen, die ein spezielles Tool, System oder eine KI-Integration benötigen, um ihre Arbeit einfacher oder zuverlässiger zu machen."
    },
    "about_p2": {
        en: "You provide the challenge and we code a custom solution that fits your needs. The software is designed around your workflow and helps reduce repetitive tasks, manual work, and unnecessary complexity. With optional AI integration, we can help you automate your workflow.",
        nl: "Jij levert de uitdaging en wij coderen een oplossing die precies aansluit bij jouw behoeften. De software wordt ontworpen rondom jouw workflow en helpt repetitieve taken, handmatig werk en onnodige complexiteit te verminderen. Met optionele AI-integratie kunnen we je helpen je workflow te automatiseren.",
        de: "Sie stellen die Herausforderung, und wir programmieren eine maßgeschneiderte Lösung, die Ihren Anforderungen entspricht. Die Software ist auf Ihren Arbeitsablauf zugeschnitten und hilft dabei, wiederkehrende Aufgaben, manuelle Arbeit und unnötige Komplexität zu reduzieren. Mit optionaler KI-Integration helfen wir Ihnen, Ihren Arbeitsablauf zu automatisieren."
    },
    "about_kicker": {
        en: "Founder & Software Builder",
        nl: "Oprichter & Softwarebouwer",
        de: "Gründer & Softwareentwickler"
    },
    "about_bio": {
        en: "With more than {years} years of programming experience and a university background in Artificial Intelligence, I combine academic AI knowledge with practical software development for SMEs.",
        nl: "Met meer dan {years} jaar programmeerervaring en een universitaire achtergrond in Kunstmatige Intelligentie combineer ik academische AI-kennis met praktische softwareontwikkeling voor het MKB.",
        de: "Mit mehr als {years} Jahren Programmiererfahrung und einem universitären Hintergrund in Künstlicher Intelligenz verbinde ich akademisches KI-Wissen mit praktischer Softwareentwicklung für den Mittelstand."
    },
    "about_speaking": {
        en: "Available for talks and workshops.",
        nl: "Beschikbaar voor presentaties en workshops.",
        de: "Verfügbar für Vorträge und Workshops."
    },
    "about_linkedin": {
        en: "Connect on LinkedIn",
        nl: "Verbind op LinkedIn",
        de: "Auf LinkedIn verbinden"
    },
    "about_headshot_alt": {
        en: "Sten Nellen",
        nl: "Sten Nellen",
        de: "Sten Nellen"
    },

    // Showcase Section
    "showcase_title": {
        en: "Research, Development & Sharing",
        nl: "Onderzoek, Ontwikkeling & Delen",
        de: "Forschung, Entwicklung & Teilen"
    },
    "showcase_subtitle": {
        en: "Alongside custom business software, we develop innovative prototypes with hardware, AI, and robotics through <strong>Creative Mindstorms</strong>. With more than 250,000 followers and open-source contributions, we demonstrate our expertise in building advanced systems.",
        nl: "Naast maatwerksoftware voor bedrijven ontwikkelen we met <strong>Creative Mindstorms</strong> innovatieve prototypes met hardware, AI en robotica. Met meer dan 250.000 volgers en open-source bijdragen tonen we onze expertise in het bouwen van geavanceerde systemen.",
        de: "Neben maßgeschneiderter Software für Unternehmen entwickeln wir mit <strong>Creative Mindstorms</strong> innovative Prototypen mit Hardware, KI und Robotik. Mit mehr als 250.000 Followern und Open-Source-Beiträgen zeigen wir unsere Expertise im Bau fortgeschrittener Systeme."
    },
    "latest_projects": {
        en: "<i class=\"fab fa-youtube\"></i> Latest Projects",
        nl: "<i class=\"fab fa-youtube\"></i> Nieuwste Projecten",
        de: "<i class=\"fab fa-youtube\"></i> Neueste Projekte"
    },
    "open_source": {
        en: "<i class=\"fab fa-github\"></i> Open Source",
        nl: "<i class=\"fab fa-github\"></i> Open Source",
        de: "<i class=\"fab fa-github\"></i> Open Source"
    },
    "check_channels": {
        en: "Check out the full collection on our channels:",
        nl: "Bekijk de volledige collectie op onze kanalen:",
        de: "Schauen Sie sich die gesamte Sammlung auf unseren Kanälen an:"
    },

    // Contact Section
    "contact_title": {
        en: "Let's Build Something Together",
        nl: "Laten We Samen Iets Bouwen",
        de: "Lassen Sie uns zusammen etwas bauen"
    },
    "contact_text": {
        en: "Ready to automate your workflow or build a custom tool?",
        nl: "Klaar om je workflow te automatiseren of een eigen tool te bouwen?",
        de: "Bereit, Ihren Workflow zu automatisieren oder ein eigenes Tool zu bauen?"
    },

    // Footer
    "footer_slogan": {
        en: "Custom Software Solutions.",
        nl: "Maatwerk Software Oplossingen.",
        de: "Maßgeschneiderte Softwarelösungen."
    },
    "footer_kvk": {
        en: "KvK: 99284596",
        nl: "KvK: 99284596",
        de: "KvK: 99284596"
    },
    "footer_vat": {
        en: "VAT: NL005379334B28",
        nl: "BTW: NL005379334B28",
        de: "MwSt: NL005379334B28"
    },
    "footer_copyright": {
        en: "&copy; <span id='year'></span> RobaLink. All rights reserved.",
        nl: "&copy; <span id='year'></span> RobaLink. Alle rechten voorbehouden.",
        de: "&copy; <span id='year'></span> RobaLink. Alle Rechte vorbehalten."
    },

    // Metadata & Alt Text
    "page_title": {
        en: "RobaLink | Custom Software & Automation",
        nl: "RobaLink | Maatwerk Software & Automatisering",
        de: "RobaLink | Individuelle Software & Automatisierung"
    },
    "meta_description": {
        en: "RobaLink builds custom software and automation for businesses that need a specific tool, system, or AI integration to make their work easier or more reliable.",
        nl: "RobaLink bouwt maatwerk software en automatisering voor bedrijven die een specifieke tool, systeem of AI-integratie nodig hebben om hun werk eenvoudiger of betrouwbaarder te maken.",
        de: "RobaLink entwickelt maßgeschneiderte Software und Automatisierungslösungen für Unternehmen, die ein spezifisches Tool, System oder eine KI-Integration benötigen, um ihre Arbeit einfacher oder zuverlässiger zu machen."
    },
    "og_title": {
        en: "RobaLink | Custom Software & Automation",
        nl: "RobaLink | Maatwerk Software & Automatisering",
        de: "RobaLink | Individuelle Software & Automatisierung"
    },
    "alt_logo": {
        en: "RobaLink Logo",
        nl: "RobaLink Logo",
        de: "RobaLink Logo"
    },

    // Dynamic Content
    "typewriter_code": {
        en: "function create() {\n  return \"Solution\";\n}",
        nl: "function create() {\n  return \"Oplossing\";\n}",
        de: "function create() {\n  return \"Lösung\";\n}"
    }
};
